...
?>
<ul class="weblinks<?php echo $moduleclass_sfx; ?> list-striped">  <?php // Wright v.4: Added list-striped class ?>
	<?php
	foreach ($list as $item) :
		...
		<li>
			...
			switch ($params->get('target', 3))
			{
				case 1:
					...
					echo '<a href="' . $link . '" target="_blank" rel="' . $params->get('follow', 'nofollow') . '">' .
						'<i class="far fa-link"></i>' .  // Wright v.4: Added icon
			...
			{
				echo /*'(' .*/ $item->hits . ' ' . JText::_('MOD_WEBLINKS_HITS') /*. ')'*/; // Wright v.4: removed parenthesis
			}
			?>
		</li>
	<?php
