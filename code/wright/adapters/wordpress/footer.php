...
class WrightAdapterWordpressFooter {

	public function render($args) {

		//TODO: handle the logic to replace the calls to get_footer from the template file
		//i.e include from /html/wordpress the footer.php (or a specialized-via-args version)


		$wright = WrightFactory::getInstance();
		$branding = '';

		if ($wright->getOption('rebrand') !== 'yes') {
			...
			$branding =  '<a class="joomlashack" href="'.$host.'">'.$anchor.$endline.'</a>'.PHP_EOL;

		}

		//capture footer contents
		ob_start();
		wp_footer();
